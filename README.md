...
The goal of this document is to provide a system overview and to outline possible deployment with applicable samples.
...
The section that follows covers the prototypical deployment approach. Feel free to adapt it to your environment and
specific
context.

## Images overview

The system consists of two main images:

* App image (used to deploy main applicative container)
* Workers image (used to deploy background workers container(s))
...
### [App image](https://gallery.ecr.aws/f8h5l8q1/adyen/hospitality/api)

This image encapsulates the main applicative component of the system. Container-based on this image must be exposed to
the internet either through a reverse proxy or through a load balancer.
The application server API and user interface, as well as connectivity with Adyen and OHIP.
...
the [assets directory](assets/docker/app/appsettings.App.json).
...
| AWS__ServiceEndpoint                    | http://elasticmq:9324                                                                                                                        | Connection details for your AWS SQS compatible queue. [exmple](https://github.com/softwaremill/elasticmq)                                                                         |
| AWS__QueueUrl                           | http://elasticmq:9324/queue/adyen-platform.fifo                                                                                              |                                                                                                                                                                                   |
| AWS__DlQueueUrl                         |                                                                                                                                              |                                                                                                                                                                                   |
...
### [Workers image](https://gallery.ecr.aws/f8h5l8q1/adyen/hospitality/workers)

This image defines a background service that processes payment notifications. Containers based upon this image must
remain in a private subnetwork. Additionally, they can be scaled out as necessary.

#### Configuration overview

The example of the file is provided in
the [assets directory](assets/docker/app/appsettings.Worker.json).
...
| AWS__ServiceEndpoint                    | http://elasticmq:9324                                                                                                                               |
| AWS__QueueUrl                           | http://elasticmq:9324/queue/adyen-platform.fifo                                                                                                     |
| AWS__DlQueueUrl                         |                                                                                                                                                     |
...
### [Migrations image](https://gallery.ecr.aws/f8h5l8q1/adyen/hospitality/migrations)
